...
-   Gérer différents RPG
-   Charge une seule fois le RPG pour toutes les cultures !
-   Attention RPG par région !
-   Join avec SAA
-   Voir : Avis : Using an external vector in selections was deprecated in tidyselect 1.1.0. Please use `all_of()` or `any_of()` instead. Was: data %\>% select(lon) Now: data %\>% select(all_of(lon)) See <https://tidyselect.r-lib.org/reference/faq-external-vector.html>.
...
```{r}
list_cult = c("01 - Blé tendre d'hiver et épeautre", "14 - Maïs grain irrigué", "15 - Maïs grain non irrgué") 
```
...
dict_SAA_to_RPG = list("01 - Blé tendre d'hiver et épeautre" = c("BTH", "BH"),
                       "14 - Maïs grain irrigué" = c("MA", "MIS"),
                       "15 - Maïs grain non irrgué" = c("MA", "MIS")
                         )
dict_RPG_to_SAA = list("BTH" = "01 - Blé tendre d'hiver et épeautre", 
                       "BH" = "01 - Blé tendre d'hiver et épeautre",
                       "MA" = c("14 - Maïs grain irrigué", "15 - Maïs grain non irrgué"),
                       "MIS" = c("14 - Maïs grain irrigué", "15 - Maïs grain non irrgué")
                       )
```
...
#### Variables gloables géographiques
...
#### Fonction pour une culture, un département et une année donnés

```{r}
# truc = as.Date("2022-01-01", format = "%Y-%m-%d") + 4
# format(truc, "%Y%m%d")
```
...
10 minutes pour une région, trois cultures et une année
Donc -> 10 minutes * 12 * 13 = 1560 minutes soit 26 heures
